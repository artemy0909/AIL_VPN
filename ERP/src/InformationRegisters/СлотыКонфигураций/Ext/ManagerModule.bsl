...
Функция ПолучитьИнформациюОКонфигурациях(Серверы=Неопределено) Экспорт
	...
	Запрос.Текст = 
		...
		|	ВЫБОР
		|		КОГДА ВТ_ЗАНЯТОСТЬ_СЕРВЕРОВ.Сервер.МаксимумСлотов = 0
		|			ТОГДА 0
		|		ИНАЧЕ (ВТ_ЗАНЯТОСТЬ_СЕРВЕРОВ.Сервер.МаксимумСлотов - (ВТ_ЗАНЯТОСТЬ_СЕРВЕРОВ.ВсегоСлотов - ВТ_ЗАНЯТОСТЬ_СЕРВЕРОВ.ДоступноСлотов)) / ВТ_ЗАНЯТОСТЬ_СЕРВЕРОВ.Сервер.МаксимумСлотов
		|	КОНЕЦ КАК КоэфициентПриоритетности
		|ИЗ
