...
Внесены исправления от 01.09.2024, ответвление от версии 1.0.1-beta
"""
...
class КонтекстОбъекта:
    ...
    def _записатьДанные(self, режимЗаписиДокумента=РежимЗаписиДокумента.Запись,
    ...
    def УстановитьСсылку(self, ссылка):
        ...
        if self.ТипКоллекции() != ссылка.ТипКоллекции() or self.ИмяКоллекции() != ссылка.ИмяКоллекции():
            raise ValueError('Тип аргумента "ссылка" не соответствует требуемому типу "{0}".'.format(
                self.__dict__["__reference"].ПолноеИмяТипа()))
    ...
    def ЗагрузитьДанные(self):
        ...
        self.Клиент().Контекст()._КонтекстДанных__установитьПредставлениеОбъекта(self.__dict__["__reference"],
                                                                                 refSoap.Presentation)
        self.Клиент().Контекст()._КонтекстДанных__установитьЗначенияИзСвойствSOAP(self.__dict__["__reference"],
                                                                                  refSoap.Property)
    ...
    def Удалить(self):
        ...
        self.Клиент().SoapКлиент.service.DeleteObject(
            self.Клиент().Сериализатор().ВЗначениеБром(self.__dict__["__reference"]))
...
class ОбъектСсылка(Ссылка):
    ...
    def ПолучитьОбъект(self):
        if self.Пустая():
            raise Exception(
                'Невозможно получить объект пустой ссылки. Воспользуйтесь методом "СоздатьОбъект" модуля менеджера коллекции.')
...
class МетаданныеКонфигурация(УзелМетаданных):
    ...
    def __init(self):
        ...
        МетаданныеКоллекция(self, self, "ПланыВидовХарактеристик", "ПланыВидовХарактеристик",
                            "Планы видов характеристик", True)
...
class МетаданныеОбъект(УзелМетаданных):
    def __init__(self, родитель, metadata):
        ...
        self.__predefinedValues = self.Клиент().Сериализатор().ИзЗначенияБром(
            metadata.Predefined) if metadata.Predefined else Структура()
...
class ОписаниеТипов:
    def __init__(self, типы, квалификаторыЧисла=None, квалификаторыСтроки=None, квалификаторыДаты=None,
                 квалификаторыДвоичныхДанных=None):
        self.__types = типы
...
class Сериализатор:
    ...
    def __ИзЗначенияБром_ПеречислениеСсылка(self, значение):
        return self.__bromClient.Контекст().ПолучитьПеречислениеСсылку(значение.Type, значение.Value,
    ...
    def __ИзЗначенияБром_ОбъектСсылка(self, значение):
        ...
        if значение.Presentation != None:
            self.__bromClient.Контекст()._КонтекстДанных__установитьПредставлениеОбъекта(reference,
                                                                                         значение.Presentation)
...
class УникальныйИдентификатор(UUID):
    ...
    def Пустой(self):
        return self == УникальныйИдентификатор('00000000-0000-0000-0000-000000000000')
...
class БромКлиент:
    ...
    def ВызватьМетод(self, имяМодуля, имяМетода, *параметры):
        return self.ВызватьМетодСМассивомПараметров(имяМодуля, имяМетода, параметры)
    ...
    def __executeQuery(self, текстЗапроса, параметры=None, отбор=None, поля=None, порядок=None, пакетный=False,
                       типОбхода=ОбходРезультатаЗапроса.Прямой, включатьВременныеДанные=False):
    ...
    def ВыполнитьЗапрос(self, текстЗапроса, параметры=None, отборы=None, поля=None, порядок=None,
    ...
    def ВыполнитьПакетныйЗапрос(self, текстЗапроса, параметры=None, отборы=None, поля=None, порядок=None,
                                типОбхода=ОбходРезультатаЗапроса.Прямой, включатьВременныеДанные=False):
        return self.__executeQuery(текстЗапроса, параметры, отборы, поля, порядок, True, типОбхода,
...
class ДокументОбъект(КонтекстОбъекта):
    ...
    def Записать(self, режимЗаписиДокумента=РежимЗаписиДокумента.Запись,
                 режимПроведенияДокумента=РежимПроведенияДокумента.Неоперативный):
...
class КонтекстДанных:
    ...
    def ПолучитьОбъектСсылку(self, полноеИмяТипа, указатель):
        ...
        if not узелМетаданных:
            raise 'Ошибка при получении ссылки на объект. Не удалось обнаружить объект метаданных "{0}".'.format(
                полноеИмяТипа)
...
class Селектор:
    ...
    def АвтозагрузкаПолей(self, value):
        self.__fieldsAutoloadSettings = value if isinstance(value,
                                                            АвтозагрузкаПолейОбъектов) else АвтозагрузкаПолейОбъектов.Ничего()
        self.__isModified = True
