...
Функция ЕстьАналогичныйСчет(Клиент, ДанныеВыбранногоТарифа)
	...
	НеоплаченыеСчета = Счета_НаСервере.НеоплаченыеСчета(Клиент);
...
Функция СчетНаОплату(ТелеграмИД, ДанныеВыбранногоТарифа, ИдентификаторМетодаОплаты) Экспорт
	...
	ДанныеОКлиенте = СессииТелеграм.ПолучитьКонтрагентаСессииИлиСоздать(
		ТелеграмИД, Перечисления.Бот.Пользовательский);
	...
	// TODO исправить price. Это должна быть базовая цена
...
Функция ПерваяОплата(ДанныеСчета) Экспорт
				
	Счет = РазвернутьОбъект(ДанныеСчета.self);	
    Счет = Счет.ПолучитьОбъект();
	ОплатаПодписки_ВызовСервера.ЗавершениеПервойОплаты(Счет);
	Подписка = Счет.Подписка.ПолучитьОбъект();
	ДанныеСлота = СерверыVPN_ВызовСервера.ЗарезервиватьСлот(Подписка.Клиент);
	
	Возврат Новый Структура("next_payment_datetime,period_text,conf",
		Подписка.ДатаСледующейОплаты(),
		НРег(Подписка.Тариф.ПериодПодписки.Метаданные().Синоним),
		Новый Структура("name,data",
			ДанныеСлота.ИмяФайла, ДанныеСлота.Файл
		)
	);
	
КонецФункции

Функция ПроверитьПромокод(Параметры) Экспорт
	
	ДанныеПромокода = Промокоды.ПолучитьДанныеПоПромокоду(Параметры.promo_code, ТекущаяДата());
	
	ДанныеОКлиенте = СессииТелеграм.ПолучитьКонтрагентаСессииИлиСоздать(
		Параметры.telegram_id, Перечисления.Бот.Пользовательский);
	Клиент = ДанныеОКлиенте.Контрагент;
	...
	ИначеЕсли ТипЗнч(ДанныеПромокода.ИсточникПодписки) = Тип("ДокументСсылка.ПодарочныеПромокоды") Тогда
		
		НоваяПодписка = Документы.Подписка.СоздатьДокумент();
		НоваяПодписка.Дата = ТекущаяДата();
		НоваяПодписка.БазоваяСтоимость = 0;
		НоваяПодписка.СуммаРегулярногоПлатежа = 0;
		НоваяПодписка.Клиент = Клиент;
		НоваяПодписка.ДатаНачала = НоваяПодписка.Дата;
		НоваяПодписка.Промокод = Параметры.promo_code;
		НоваяПодписка.Тариф = Справочники.Тариф.Бесплатный;
		НоваяПодписка.Статус = Перечисления.СтатусПодписки.Активна;
		НоваяПодписка.ИсточникПодписки = ДанныеПромокода.ИсточникПодписки;
		
		ДанныеСлота = СерверыVPN_ВызовСервера.ЗарезервиватьСлот(НоваяПодписка.Клиент);
		Если ДанныеСлота = Неопределено Тогда
			Возврат HTTP_Соединение.СформироватьОтвет(Неопределено, 404);
		КонецЕсли;
		
		НоваяКонфигурация = НоваяПодписка.Конфигурации.Добавить();
		НоваяКонфигурация.Сервер = ДанныеСлота.Сервер;
		НоваяКонфигурация.ИмяКонфигурации = ДанныеСлота.ИмяКонфигурации;
		НоваяКонфигурация.ИдСлота = ДанныеСлота.ИдСлота;
		НоваяПодписка.Записать(РежимЗаписиДокумента.Проведение, РежимПроведенияДокумента.Оперативный);
		
		ПараметрыОтвета = Новый Структура("next_payment_datetime,period_text,conf",
			Неопределено,
			НРег(НоваяПодписка.Тариф.ПериодПодписки.Метаданные().Синоним),
			Новый Структура("name,data",
				ДанныеСлота.ИмяФайла, ДанныеСлота.Файл
			)
		);
		
		Возврат HTTP_Соединение.СформироватьОтвет(ПараметрыОтвета, 201);

	Иначе
		
		ПараметрыОтвета = ПрайсЛистПоПромокоду(ДанныеПромокода, Параметры.promo_code);
