...
Функция promocode_infoGET(Запрос)
	...
	ДанныеПромокода = Промокоды.ПолучитьДанныеПоПромокоду(ПромокодСтрока, ДатаПодписки);
...
Функция userPOST(Запрос)
	...
	Ответ = СессииТелеграм.ПолучитьКонтрагентаСессииИлиСоздать(
		ПараметрыЗапроса.telegram_id, Перечисления.Бот.Пользовательский, ПараметрыЗапроса.full_name);
...
Функция paymentPOST(Запрос)
	...
	ПараметрыЗапроса = HTTP_Соединение.ПолучитьПараметрыОтвета(Запрос);
	ПараметрыОтвета = КсенонОтвет.ПерваяОплата(ПараметрыЗапроса.invoice);
	Возврат HTTP_Соединение.СформироватьОтвет(ПараметрыОтвета); 
	
КонецФункции

//Функция check_start_infoPOST(Запрос)
//	
//	ПараметрыЗапроса = HTTP_Соединение.ПолучитьПараметрыОтвета(Запрос);
//	//ПараметрыОтвета = 
//	Возврат HTTP_Соединение.СформироватьОтвет(ПараметрыОтвета);
