...
Функция invoicePOST(Запрос)
	...
	ПараметрыЗапроса = ПолучитьПараметрыЗапроса(Запрос);
	Ответ = КсенонОтвет.СчетНаОплату(ПараметрыЗапроса.telegram_id, ПараметрыЗапроса.selected_item);
...
Функция paymentPOST(Запрос)
	...
	ПараметрыЗапроса = ПолучитьПараметрыЗапроса(Запрос);
	КсенонОтвет.Оплата(ПараметрыЗапроса.invoice);
	Возврат СформироватьОтвет(Неопределено); 
