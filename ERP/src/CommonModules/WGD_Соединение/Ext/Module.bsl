...
Функция СоздатьСлотКонфигурации(ИмяКонфигурации, СерверСсылка, ИмяСлота) Экспорт
	
	// TODO рассмотреть возможность preshared_key
	ПараметрыЗапроса = Новый Структура(
	...
	Возврат СформироватьЗапросWGD(
		СерверСсылка, "POST", "/addPeers/" + ИмяКонфигурации, ПараметрыЗапроса
	);
